...
function rollDice() {
    if (document.getElementById('name-' + activePlayer).textContent == 'Winner!') {} else {
        ...
        document.querySelector('.dice').src = 'img/dice-' + randNum + '.png';
        if (randNum === 1) {
            playerCurrentScore.textContent = 0;
            toggleActivePlayer();
        } else {
            playerCurrentScore.textContent = +playerCurrentScore.textContent + randNum;
        }
    }
}
...
function hold() {
    ...
    if (Number(palyerTotalScore) >= 10) {
        isWinner();
    } else if (Number(palyerTotalScore) === 0) {
